// FALLBACK: browser-based size scraper.
//
// Superseded by fetch-sizes.js, which reads the same data from Uniqlo's JSON API
// without a browser. Kept as a working fallback in case the API changes shape or
// the x-fr-clientid header stops being accepted.
//
// Verified working 2026-08-25 against live DE sale products. Requires the
// anti-fingerprint setup in preparePage(): Uniqlo's Akamai edge returns a 403
// "Access Denied" page to anything that fingerprints as headless Chrome.
//
// Usage: node fetch-sizes-browser.js --country de --limit 100
...
const CONCURRENCY = 5;
const BATCH_SIZE = 5;
// Hard cap per product. Without this one wedged tab stalls the whole Promise.all
// batch forever and the run goes silent.
const PRODUCT_TIMEOUT_MS = 120000;
...
// Akamai 403s the product pages when the request looks like headless Chrome.
// Overriding the UA string alone is not enough - the Sec-CH-UA client hints still
// announce "HeadlessChrome", so the UA metadata has to be set too. `platform` must
// be passed as well, otherwise navigator.platform keeps the host value and reports
// "Linux x86_64" on the CI runner while the UA claims macOS.
...
async function readColorAndSizes(page, colorLabel, expectedColorCode) {
  ...
  let sizes = [];
  try {
    // .size-chip-group renders before the chips inside it, and the chip labels land a
    // tick after the wrapper element does - wait for a wrapper that actually has text.
    await page.waitForFunction(() => {
      return Array.from(document.querySelectorAll('.size-chip-wrapper'))
        .some(w => (w.querySelector('button')?.textContent || '').trim().length > 0);
    }, { timeout: 10000 });
    ...
    sizes = await page.evaluate(() => {
      ...
      return wrappers
        ...
        .map(wrapper => {
          // textContent, not innerText: innerText needs layout and comes back empty
          // when the chips have only just been inserted.
...
async function visitAndRead(page, url, colorLabel, productName) {
  try {
    ...
    // Akamai serves a 403 "Access Denied" page to blocked clients. Without this the
    // selectors just come back empty and the product silently reads as Unavailable.
    if (response && response.status() >= 400) {
      console.error(`  [${productName}] BLOCKED HTTP ${response.status()} (${url})`);
      blockedRequests++;
      return null;
    }
...
(async () => {
  ...
  for (let i = 0; i < total; i += CONCURRENCY) {
    ...
    await Promise.all(batch.map(row =>
      withTimeout(processProduct(row), PRODUCT_TIMEOUT_MS, `[${row['Product Name']}]`)
        .catch(err => {
          console.error(`  [${row['Product Name']}] Aborted: ${err.message}`);
          if (!row['Available Sizes']) row['Available Sizes'] = 'Unavailable';
        })
    ));
  }
  ...
  if (blockedRequests > 0) {
    console.error(`WARNING: ${blockedRequests} request(s) were blocked by the site (HTTP 4xx/5xx).`);
  }
})();
